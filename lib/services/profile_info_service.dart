  ...
  updateProfileInfo(
    ...
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('$baseApi/user/update-profile'),
      );
      ...
      if (selectedImage != null) {
        print('Attempting to upload image: ${selectedImage!.path}');
        if (await selectedImage!.exists()) {
          var fileSize = await selectedImage!.length();
          print('File size: $fileSize bytes');
          if (fileSize > 2 * 1024 * 1024) {
            showToast(
              asProvider.getString('Image size exceeds 2MB'),
              cc.red,
            );
          ...
          print('File added to request: ${selectedImage!.path}');
        } else {
          print('Selected image does not exist: ${selectedImage!.path}');
          showToast(
            asProvider.getString('Selected image is invalid'),
            cc.red,
          );
          setLoadingProfileUpdate(false);
          return;
      ...
      if (selectedImage != null) {
        snackBar(
          context,
          asProvider.getString('Uploading image, this might take some time...'),
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Profile update response: ${await response.stream.bytesToString()}');
        ...
        print('Profile update failed: ${response.reasonPhrase}');
        print('Response body: ${await response.stream.bytesToString()}');
        showToast(
          asProvider.getString('Profile info update failed'),
          cc.red,
        );
        setLoadingProfileUpdate(false);
      }
      ...
      showToast(asProvider.getString('Error: $err'), cc.red);
      setLoadingProfileUpdate(false);
    }
  }
}
