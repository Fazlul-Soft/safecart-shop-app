...
import 'package:safecart/services/location/country_dropdown_service.dart';
import 'package:safecart/services/location/state_dropdown_service.dart';
import 'package:safecart/views/product_by_subcategory_view.dart'; // Add this import
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              const Text(
                'An error occurred with the app, please contact the developer',
                style: TextStyle(
                  color: Colors.orangeAccent,
                  fontWeight: FontWeight.bold,
                ),
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(create: (context) => AllProductsService()),
        ChangeNotifierProvider(create: (context) => InternetCheckerService()),
        ChangeNotifierProvider(create: (context) => ProductByCampaignsService()),
        ChangeNotifierProvider(create: (context) => HomeCampaignProductsService()),
        ChangeNotifierProvider(create: (context) => SocialSignInSignUpService()),
      ],
      child: Consumer<RTLService>(builder: (context, rtlProvider, child) {
        return MaterialApp(
          title: 'Sombokchab',
          ...
          builder: (context, rtlchild) {
            return Directionality(
              textDirection: rtlProvider.langRtl ? TextDirection.rtl : TextDirection.ltr,
              child: rtlchild!,
            );
          },
          debugShowCheckedModeBanner: false,
          ...
          supportedLocales: [
            Locale(rtlProvider.slug?.substring(0, 2) ?? "en", rtlProvider.slug?.substring(3, 4) ?? "GB")
          ],
          theme: ThemeData(
            primarySwatch: Colors.blue,
            textTheme: TextTheme(
            ...
            radioTheme: DefaultThemes().radioThemeData(context, cc),
            appBarTheme: DefaultThemes().appBarTheme(context, cc),
            elevatedButtonTheme: DefaultThemes().elevatedButtonTheme(context, cc),
            outlinedButtonTheme: DefaultThemes().outlinedButtonTheme(context, cc),
            inputDecorationTheme: DefaultThemes().inputDecorationTheme(context, cc),
            checkboxTheme: DefaultThemes().checkboxTheme(context, cc),
            scaffoldBackgroundColor: Colors.white,
          ),
          home: const SplashView(),
          routes: {
            ...
            NewTicketView.routeName: (context) => NewTicketView(),
            HomeCampaignsView.routeName: (context) => HomeCampaignsView(),
            ProductByCampaignView.routeName: (context) => ProductByCampaignView(),
            ShippingAddressListView.routeName: (context) => ShippingAddressListView(),
            ProductByCategoryView.routeName: (context) => ProductByCategoryView(),
            ChangePasswordView.routeName: (context) => ChangePasswordView(),
            OrderConfirmationScreen.routeName: (context) => OrderConfirmationScreen(),
          },
          onGenerateRoute: (settings) {
            if (settings.name == ProductBySubcategoryView.routeName) {
              final args = settings.arguments as List?;
              final subcatId = args?.first as int?;
              if (subcatId != null) {
                return MaterialPageRoute(
                  builder: (context) => ProductBySubcategoryView(),
                  settings: settings,
                );
              }
              return MaterialPageRoute(
                builder: (context) => Scaffold(
                  body: Center(child: Text('Invalid subcategory ID')),
                ),
              );
            }
            return null;
